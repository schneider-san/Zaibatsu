...
func (httpResponse *HTTPResponse) PatchHeaders(p *ReverseProxy) {
    ...
    if p.Terminate {
        log.Debugf("Terminating session for %s", p.RequestContext.UserID)
        p.RequestContext.InvokeTerminateUserHooks(p.RequestContext.UserID)
        //log.Debugf("Terminating session for %s", p.RequestContext.IP)
        //p.RequestContext.InvokeTerminateUserHooks(p.RequestContext.IP)

        // Set Terminator Cookie
        value := runtime.TERMINATE_SESSION_COOKIE_NAME + "=" + runtime.RandSeq(32) +
            ";Path=/;Domain=." + runtime.ProxyDomain +
